  ...
  <div class="multi-agent-view">
    <div class="content-container" :class="{ 'sidebar-collapsed': sidebarCollapsed }">
      ...
      <div class="chat-container">
        ...
        <div class="sidebar-toggle-container" v-if="sidebarCollapsed">
          <div class="sidebar-buttons-wrapper">
            <el-button @click="toggleSidebar" type="text" class="sidebar-btn">
              <div class="ds-icon">
                <svg
                  width="16"
                  height="16"
            ...
            <el-button @click="newConversation" type="text" class="sidebar-btn">
              <div class="ds-icon">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
        ...
        <div class="chat-messages" ref="messagesContainer">
          <div class="chat-messages-content">
                ...
                <div class="loading-content">
                  <div class="step-indicator">
                    ...
                    <Transition name="text-switch" mode="out-in">
                      <span class="loading-text" :key="currentStep">{{
                        currentStep || '处理中...'
                      }}</span>
                    </Transition>
                  </div>
            ...
            <div
              v-if="messages.filter(m => !m.isWelcome).length === 0 && !loading"
            ...
            <div
              v-if="messages.filter(m => !m.isWelcome).length === 0 && !loading"
              class="centered-input-container"
            >
              <FloatingInput
                :loading="loading"
            ...
            if (data.type === 'step') {
              ...
              if (sqlResult.success) {
                ...
                ElNotification.error({
                  title: '失败',
                  message: sqlResult.error_message,
                  duration: 3000
                })
              }
  ...
  try {
    ...
    if (history.length > 0) {
      ...
      messages.value.push({
        ...
        content:
          '你好！我是多智能体 SQL 助手。请输入您的自然语言查询，我会帮您生成相应的 SQL 语句。',
...
const handleEdit = async (index, newContent) => {
  ...
  const hasAssistantReply =
    index + 1 < nonWelcomeMessages.length && nonWelcomeMessages[index + 1].role === 'assistant'

  // 删除该用户消息和对应的助手消息（如果存在）

  messages.value = messages.value.filter((msg, msgIndex) => {
    const filteredIndex = messages.value
      .slice()
...
/* 内容容器收起侧边栏状态 */
.content-container.sidebar-collapsed .chat-container {
  margin-left: 0;
  width: 100%;
}
...
.welcome-icon {
  ...
  background: linear-gradient(135deg, #0b2af3 0%, #02a0ff 100%);
